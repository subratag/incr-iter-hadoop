...
Kmeans is a commonly used clustering algorithm. We describe the algorithm briefly as follows. (1) Start with selecting $k$ random points as cluster centroids, (2) Assign each point to the nearest centroid, (3) Update the $k$ centroids by ``averaging'' the points belonging to the same cluster. Repeat steps (2) and (3) until convergence has been reached. In Kmeans, the points set is the structure data (non-changed during iteration). Each point is represented by <pid,pv>, where point id pid is the key and point value pv is the point coordinate. The centroids set is the state data. Since assigning any point to a cluster requires knowing the whole centroids set, the whole centroids set is a single state kv <**1**,cvs>. The state key **1** is a global unique key and the state value cvs contains all centroids' coordinates (each centroid's id is $cid$ and the coordinate is $cv$). The correlation between structure kv and state kv is **all-to-one**.
